...
### Excluding tasks from groups

It's possible to exclude tasks from user groups, which is only effective during task entry (doesn't effect the reports). When a task is excluded, it's not displayed in Slack or in the web app during time entry. Useful for teams who has crowded task lists.

```yaml
default_task_modifiers: &default_task_modifiers
  deny_list:
    - groups:
      - "sr. designers"
      - "mid. designers"
...
After adding this to your platform-wide metadata, call if from your project-specific metadata, like:

```yaml
task_modifiers: *default_task_modifiers
```
